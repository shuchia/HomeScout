...
| 23 | open | ops | **Resend email never fully set up.** `RESEND_API_KEY` is empty in `snugd/qa/secrets`, `ALERT_FROM_EMAIL` unset, and `snugd.ai` has no Resend SPF/DKIM — so Pro daily-alert emails (`tasks/alert_tasks.py`) aren't DNS-authenticated and likely spam/fail. Fix (recommended): add **`send.snugd.ai`** as a sending subdomain in the Resend dashboard (keeps root SPF Zoho-only, no merge), add the MX/SPF/DKIM it returns to Route 53 (zone `Z0395473NLNZEU6NWI7P`), then set real `RESEND_API_KEY` + `ALERT_FROM_EMAIL=alerts@send.snugd.ai` in `snugd/{env}/secrets`. Alt: verify root domain and merge SPF to `v=spf1 include:zoho.com include:amazonses.com ~all`. Note: Zoho mail (root MX/SPF/DKIM `zmail` selector) + DMARC `p=none` are already live as of 2026-06-19. Deferred by user. |
