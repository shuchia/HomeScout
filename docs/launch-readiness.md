...
| 24 | open | product | Swap Commute-address autocomplete (Settings → Commute Addresses) from free Nominatim/OSM to **Google Places Autocomplete** (Maps key already provisioned). Nominatim whiffs on building names ("The Tower at PNC Plaza" → 0 results) and picks the wrong city for bare street addresses ("300 Fifth Avenue" → Waltham MA instead of Pittsburgh) — users must hand-type "street, City, ST". Places fixes both (location-biased, ranked candidates, resolves business/building names). Keep it cheap: debounce + session tokens. Contained change — `frontend/components/CommuteAddresses.tsx` input + `frontend/lib/geocode.ts` (or a small backend proxy to keep the key server-side). |
